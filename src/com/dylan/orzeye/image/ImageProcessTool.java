...
public class ImageProcessTool {
	...
	public static Bitmap getOCRBitmapFromCamera(byte[] data, Camera camera, DisplayMetrics dm) {
		...
		YuvImage image = new YuvImage(data, imageFormat, imageSize.width, imageSize.height, null);
		if (image != null) {
			try {
				ByteArrayOutputStream stream = new ByteArrayOutputStream();
				image.compressToJpeg(new Rect(0, 0, imageSize.width, imageSize.height),100, stream);
			...
			double heightRatio = (double)rotaBitmap.getHeight()/(double)dm.heightPixels;
			double widthRatio = (double)rotaBitmap.getWidth()/(double)dm.widthPixels;
